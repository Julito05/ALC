# -*- coding: utf-8 -*-
"""
Created on Mon Apr  7 22:18:29 2025
...
def proyectarPts(T, wz):
    assert(T.shape == (3,3)) # chequeo de matriz 2x2
    assert(wz.shape[0] == 2 or wz.shape[0] == 3) # multiplicacion matricial valida   
    
    if wz.shape[0] == 2:
        ones = np.ones((1, wz.shape[1]))
        wz = np.vstack((wz, ones))  # agregar fila de unos para coordenadas homogéneas

    
    xy = None
    ############### Insert code here!! ######################3    
    xy_h = T @ wz
    # Volver a coordenadas cartesianas (dividir por la tercera fila si no es 1)
    xy = xy_h[:2] / xy_h[2]
...
##############
def puntosCircunferencia(radio=40, n_puntos=200, centro=(50, 50)):
    t = np.linspace(0, 2 * np.pi, n_puntos)
    x = centro[0] + radio * np.cos(t)
    y = centro[1] + radio * np.sin(t)
    wz = np.vstack((x, y))  # matriz 2 x N
    return wz
##############
...
def main():
    ...
    T = pd.read_csv('TrotCont.csv', header=None).values
    ...
    #wz = puntosCircunferencia(radio=40, centro=(50, 50))
